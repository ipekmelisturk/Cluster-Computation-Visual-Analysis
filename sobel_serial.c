...
int main(int argc, char **argv) {
	...
	new_image.pixels=malloc(image.xsize*image.ysize*image.depth*sizeof(char));

	/* Allocate space for output image */
	sobel_x.xsize=image.xsize;
	sobel_x.ysize=image.ysize;
	sobel_x.depth=image.depth;
	sobel_x.pixels=malloc(image.xsize*image.ysize*image.depth*sizeof(char));

	/* Allocate space for output image */
	sobel_y.xsize=image.xsize;
	sobel_y.ysize=image.ysize;
	sobel_y.depth=image.depth;
	sobel_y.pixels=malloc(image.xsize*image.ysize*image.depth*sizeof(char));
